...
from transformer.AttentionBlock import Attention
from transformer.activation_function import SnakeBeta
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        in_channels,
        out_channels,
        downsampling_upsampling_channels=(256, 256),
        num_mid_blocks=2,
        dropout=0.05,
        n_transformer_per_block=1,
        attention_head_dim=64,
        num_attention_heads=4,
    ):
        super().__init__()
        downsampling_upsampling_channels = tuple(downsampling_upsampling_channels)
        ...
        for i in range(len(downsampling_upsampling_channels)):   # are the channel progession for example (2C, 256, 512)
            ...
            transformers = nn.ModuleList([
                Transformer(output_channel, num_attention_heads, attention_head_dim, dropout)
                for _ in range(n_transformer_per_block)  # for our case is only one (is the number of transformer block in each level)
            ])
        ...
        for _ in range(num_mid_blocks):
            resnet = ResnetBlock1D(downsampling_upsampling_channels[-1], downsampling_upsampling_channels[-1], time_embed_dim)
            transformers = nn.ModuleList([
                Transformer(downsampling_upsampling_channels[-1], num_attention_heads, attention_head_dim, dropout)
        ...
        # ========== UP BLOCKS ==========
        channels_reversed = downsampling_upsampling_channels[::-1] + (downsampling_upsampling_channels[0],)
    ...
    def forward(self, x, mask, mu, t):
        ...
        velocity = self.final_proj(x * mask)
...
if __name__ == '__main__' : 
    ...
    model = UNet(
        in_channels=80,
        out_channels=80,
        downsampling_upsampling_channels=(256, 256, 512),
        dropout=0.1,
        attention_head_dim=64,
